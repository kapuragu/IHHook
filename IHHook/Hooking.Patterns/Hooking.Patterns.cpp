...
ptrdiff_t details::get_process_base() { return ptrdiff_t(GetModuleHandle(nullptr)); }
...
class executable_meta
{
  private:
    ...
    template <typename TReturn, typename TOffset> TReturn* getRVA(TOffset rva) { return (TReturn*) (m_begin + rva); }
  ...
  public:
    explicit executable_meta(uintptr_t module)
        ...
        , m_end(0)
    {
        static auto getSection = [](const PIMAGE_NT_HEADERS nt_headers, WORD section) -> PIMAGE_SECTION_HEADER { return reinterpret_cast<PIMAGE_SECTION_HEADER>((UCHAR*) nt_headers->OptionalHeader.DataDirectory + nt_headers->OptionalHeader.NumberOfRvaAndSizes * sizeof(IMAGE_DATA_DIRECTORY) + section * sizeof(IMAGE_SECTION_HEADER)); };
